  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <LinearGradient colors={["#32003F", "#4C007A"]} style={styles.header}>
          <ThemedText type="title" style={styles.headerText}>
            Order Successful
          </ThemedText>
        </LinearGradient>
        <ThemedText style={styles.orderNumber}>Order Number: {order.orderNumber}</ThemedText>

        <View style={styles.table}>
          <View style={styles.tableRow}>
            <ThemedText style={[styles.tableHeader, { flex: 1 }]}>Qty</ThemedText>
            <ThemedText style={[styles.tableHeader, { flex: 3 }]}>Product</ThemedText>
            <ThemedText style={[styles.tableHeader, { flex: 2 }]}>Unit Price</ThemedText>
            <ThemedText style={[styles.tableHeader, { flex: 2 }]}>Total</ThemedText>
          </View>
          {order.items.map((item) => (
            <View key={item.id} style={styles.tableRow}>
              <ThemedText style={[styles.tableCell, { flex: 1 }]}>{item.quantity}</ThemedText>
              <ThemedText style={[styles.tableCell, { flex: 3 }]}>{item.productName}</ThemedText>
